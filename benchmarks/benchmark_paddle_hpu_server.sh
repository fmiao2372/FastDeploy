...
# rm -rf log 2>/dev/null
# HPU_PERF_BREAKDOWN_SYNC_MODE=1 HPU_WARMUP_BUCKET=1 HPU_WARMUP_MODEL_LEN=3072 FD_ATTENTION_BACKEND=BLOCK_ATTN python -m fastdeploy.entrypoints.openai.api_server --model /data/disk3/ernie_opensource/ERNIE-4.5-300B-A47B-Paddle --port ${SERVER_PORT} --engine-worker-queue-port ${ENGINE_WORKER_QUEUE_PORT} --metrics-port ${METRICS_PORT} --tensor-parallel-size 8 --max-model-len 32768 --max-num-seqs 128 --block-size 128 --static-decode-blocks 8 --num-gpu-blocks-override 3100 --kv-cache-ratio 0.991 --enable-tensor-or-expert-parallel
