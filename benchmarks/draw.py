...
def draw_time_graph(log_dir, log_filename, max_num_seqs, mode):
    ...
    pattern_model = re.compile(r'(\d+-\d+-\d+ \d+:\d+:\d+,\d+) .* Model execution time\(ms\): ([\d\.]+), BT=(\d+), block_list_shape=\[(\d+)\], block_indices_shape=\[(\d+)\]')
    pattern_pp = re.compile(r'(\d+-\d+-\d+ \d+:\d+:\d+,\d+) .* PostProcessing execution time\(ms\): ([\d\.]+), BT=(\d+)')
    # 读取日志文件
    ...
    if mode == "all":
        ...
        with open(os.path.join(log_dir, csv_filename), 'w', newline='') as csvfile:
            ...
            writer.writerow(['Timestamp', 'ModelTime(ms)', 'BT', 'block_list_shape', 'block_indices_shape', 'Timestamp', 'PostProcessing(ms)', 'BT'])
